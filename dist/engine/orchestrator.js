...
import { getArtifactPaths, ensureArtifactDirs, writeFailure, writeDiffstat, writeSummary, writeSummaryToObsidian, } from '../artifacts/artifact-writer.js';
...
export async function runTask(record, config) {
    ...
    const planResult = await runPlanner(record.spec_path, record.worktree_path, config, plannerAdapter, paths);
    ...
    while (current.status === 'EXECUTING' || current.status === 'REPAIRING') {
        ...
        if (current.status === 'REPAIRING') {
            ...
            const repairResult = await runRepairer(executionPlan, failureOutput, violations, record.worktree_path, config, implementerAdapter, paths, attemptNumber, current.retries_remaining);
        ...
        if (current.status === 'EXECUTING') {
            ...
            const implementerAdapter = await getAdapter(config.models.implementer);
            const execResult = await runExecutor(executionPlan, record.worktree_path, config, implementerAdapter, paths, attemptNumber);
        ...
        if (current.status === 'VERIFYING') {
            ...
            if (!policyResult.passed) {
                ...
                attemptNumber++;
                if (tp.action === 'ESCALATE')
                    return current;
                continue;
            }
            // Verification gates
            const verifyResult = await runVerification(record.worktree_path, config.commands, paths, attemptNumber);
        ...
        if (!['EXECUTING', 'REPAIRING', 'VERIFYING'].includes(current.status))
            break;
    }
    // --- REVIEWING ---
    if (current.status === 'REVIEWING') {
        const diffstat = await getDiffStat(record.worktree_path);
        const reviewerAdapters = await Promise.all(config.models.reviewers.map(async (name) => ({
            name,
            adapter: await getAdapter(name),
        })));
        const reviewResult = await runReviewers(reviewerAdapters, record.spec_path, record.worktree_path, paths, diffstat);
        ...
        writeSummaryToObsidian(config.global.obsidian_vault, current.task_id, readFileSync(paths.summaryFile, 'utf8'));
    }
