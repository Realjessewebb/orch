...
export function parseDiffStat(diffStatOutput, changedFilePaths) {
    // Summary line format: "X files changed, Y insertions(+), Z deletions(-)"
    const lines = diffStatOutput.split('\n');
    const summary = lines.slice().reverse().find((l) => l.includes('changed'));
    let insertions = 0;
    let deletions = 0;
    if (summary) {
...
export function checkDiffLimits(stats, policy) {
    const violations = [];
