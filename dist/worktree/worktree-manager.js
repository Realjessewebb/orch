...
export async function createWorktree(repoPath, worktreePath, branch) {
    ...
    for (const base of baseBranches) {
        const result = await exec('git', ['worktree', 'add', worktreePath, '-b', branch, base], { cwd: repoPath });
    ...
    if (!created) {
        ...
        const result = await exec('git', ['worktree', 'add', worktreePath, '-b', branch], { cwd: repoPath });
        if (result.exitCode !== 0) {
...
export async function getDiffStat(worktreePath) {
...
export async function getChangedFiles(worktreePath) {
    const result = await exec('git', ['diff', '--name-only', 'HEAD'], { cwd: worktreePath });
    return result.stdout
        .split('\n')
        .map((l) => l.trim())
        .filter(Boolean);
